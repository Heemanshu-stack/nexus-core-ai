...
function runAutonomousSwarmSimulation(prompt) {
    ...
    appendLog("NexusCore", `Mission initialized via Autonomous Swarm Engine. ID: ${taskId}`);
...
<head>
    <meta charset="UTF-8">
    <title>Nexus Core AI - Live Generated App</title>
    <style>body { background:#080b14; color:#f8fafc; font-family:sans-serif; display:flex; justify-content:center; align-items:center; min-height:100vh; margin:0; }</style>
</head>
<body>
    <div style="background:rgba(15,23,42,0.85); padding:32px; border-radius:20px; border:1px solid #38bdf8; text-align:center; box-shadow:0 0 30px rgba(56,189,248,0.3);">
        <h1 style="color:#38bdf8; margin-bottom:12px;">? Module Ready</h1>
        <p style="color:#94a3b8; margin-bottom:20px;">Synthesized application for: ${prompt}</p>
        <a href="/" style="padding:10px 20px; background:#6366f1; color:white; border-radius:8px; text-decoration:none; font-weight:600;">Return to Dashboard</a>
    </div>
</body>
</html>`;
    ...
    setTimeout(() => {
        updateUIWithSession(simulatedSession);
        setSystemStatus("Action Required ??", "status-waiting");
        const btnSubmit = document.getElementById("btn-submit-task");
        if (btnSubmit) {
            btnSubmit.disabled = false;
